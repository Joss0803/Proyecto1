function agregarTarea() {
    ...
    if (textoTarea !== "") {
        ...
        var tareaCompletadaCheckbox = document.createElement("input");
        tareaCompletadaCheckbox.type = "checkbox";
        tareaCompletadaCheckbox.addEventListener("change", function() {
            if (tareaCompletadaCheckbox.checked) {
                itemTarea.style.textDecoration = "line-through";
            } else {
                itemTarea.style.textDecoration = "none";
            }
        });

        itemTarea.appendChild(tareaCompletadaCheckbox);
...
function filtrarTareasCompletadas() {
    var listaTareas = document.getElementsByClassName("task-item");
    for (var i = 0; i < listaTareas.length; i++) {
        var tarea = listaTareas[i];
        var checkbox = tarea.querySelector("input[type='checkbox']");
        if (!checkbox.checked) {
...
function filtrarTareasNoCompletadas() {
    var listaTareas = document.getElementsByClassName("task-item");
    for (var i = 0; i < listaTareas.length; i++) {
        var tarea = listaTareas[i];
        var checkbox = tarea.querySelector("input[type='checkbox']");
